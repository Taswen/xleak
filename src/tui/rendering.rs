...
impl TuiState {
    pub fn render(&mut self, frame: &mut Frame) {
        ...
        let max_status_height = (area.height as usize * 3 / 4).max(1) as u16;
        ...
        // Sticky header: optional column-letter row (unless --no-column-id) stacked
        // above the optional title row (unless --no-header).
        ...
        // Empty placeholder for the row-number column, sized to the header height.
        if show_row_id {
            header_cells.push(
                Cell::from(vec![Line::from(""); header_row_count.max(1) as usize]).style(
                    Style::default()
                        .fg(colors.header_fg)
